...
export async function getEntryBySlug(
  ...
  try {
    ...
    const dateValue = data.date instanceof Date ? data.date.toISOString() : (data.date || new Date().toISOString())
...
export async function getAllEntries(kind: EntryKind, language: "en" | "ar"): Promise<ContentEntry[]> {
  const slugs = getSlugs(kind, language)
  const posts = await Promise.all(slugs.map((slug) => getEntryBySlug(kind, slug, language)))
  return posts
